<template>
    <div class="city_body">
		...
		<div class="city_index">
			<ul>
				<li v-for="(item,index) in cityList" :key="item.index" @touchstart="handerToScroll(index)">{{ item.index }}</li>
			</ul>
		</div>
  </div>
</template>
	...
	mounted(){
		...
		if(cityList && hotList){
           ...
           this.axios.get('/api/cityList').then((res) => {
			// [{index:'A',list:[{nm:'北京',id：'121212'}] }]
			this.isLoading = false
			let citys = res.data.data.cities
			let {hotList,cityList} = this.formatCityList(citys)
	...
	methods : {
		formatCityList(citys){
		  ...
		  for(var i=0; i<citys.length; i++){
			  ...
			  if(toCom(firstLetter)){ //如果没有字母则添加
	  ...
	  handerToScroll(index){
		  var h2 = this.$refs.city_sort.getElementsByTagName('h2')
		//   this.$refs.city_sort.parentNode.scrollTop = h2[index].offset Top
